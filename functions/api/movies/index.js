...
// POST /api/ratings {user_id, movie, rating, note} -> upsert a rating (and cache the movie)
...
export async function onRequestGet({ request, env }) {
  ...
  const { results } = await env.DB.prepare(
    `SELECT r.id, r.rating, r.note, r.watched_at, u.id as user_id, u.name, u.color
     FROM ratings r JOIN users u ON u.id = r.user_id
     WHERE r.movie_tmdb_id = ?
     ORDER BY r.watched_at DESC`
  ).bind(movieId).all();

  return Response.json({ ratings: results });
}

export async function onRequestPost({ request, env }) {
  const body = await request.json().catch(() => ({}));
  const { user_id, movie, rating, note } = body;
  ...
  // Cache the movie the first time it's rated by anyone.
  ...
  await env.DB.prepare(
    `INSERT INTO ratings (movie_tmdb_id, user_id, rating, note, watched_at, updated_at)
     VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))
     ON CONFLICT(movie_tmdb_id, user_id) DO UPDATE SET
       rating = excluded.rating,
       note = excluded.note,
       updated_at = datetime('now')`
  ).bind(movie.tmdb_id, user_id, rating, note || null).run();

...
export async function sendPush(subscription, payload, env) {
  ...
  // TEMPORARY — attach the push service's response body so failures are
  // diagnosable; sendToUsers only reads .status once this is confirmed working.
  res._debugText = await res.text().catch(() => '');
  return res;
}

/* Fans out to many subscriptions and deletes the dead ones. */
export async function sendToUsers(env, userIds, payload) {
  if (!userIds || !userIds.length) return { sent: 0, attempts: [] };
  ...
  let sent = 0;
  const dead = [];
  // TEMPORARY debug detail — trimmed back down to { sent } once delivery works.
  const attempts = [];

  for (const sub of results) {
    try {
      const res = await sendPush(sub, payload, env);
      attempts.push({ endpoint: sub.endpoint.slice(-12), status: res.status, body: res._debugText });
      if (res.status === 404 || res.status === 410) dead.push(sub.id);
      else if (res.status >= 200 && res.status < 300) sent++;
    } catch (e) {
      attempts.push({ endpoint: sub.endpoint.slice(-12), error: String(e && e.message || e) });
    }
  }
